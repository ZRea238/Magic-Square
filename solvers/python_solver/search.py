...
from .constraints import select_next_cell_with_candidates, valid_candidates_for_cell
...
def search_first_solution_with_propagation(
    grid: Grid,
    target: int,
    size: int,
    row_sums: list[int],
    ...
    forced_assignments: list[tuple[int, int, int]] = []
    while True:
        choice = select_next_cell_with_candidates(
            target,
            size,
        ...
        r, c, candidates = choice
        if not candidates:
            message = f"{indent(depth)}Propagation detected contradiction at ({r}, {c})"
            trace(trace_enabled, trace_log, message)
            record_step("prune_branch", message, row=r, col=c)
            for rr, cc, value in reversed(forced_assignments):
                revert_value(
                    value,
                    rr,
                    cc,
        ...
        forced_value = candidates[0]
        message = f"{indent(depth)}Propagate forced value {forced_value} at ({r}, {c})"
        trace(trace_enabled, trace_log, message)
        record_step("propagate_forced", message, row=r, col=c, value=forced_value, candidates=candidates)
        apply_value(
            forced_value,
            r,
            c,
        ...
        forced_assignments.append((r, c, forced_value))
    ...
    for value in candidates:
        ...
        if search_first_solution_with_propagation(
            grid=grid,
            target=target,
            size=size,
            row_sums=row_sums,
...
def search_first_solution_exhaustive(
    grid: Grid,
    target: int,
    size: int,
    row_sums: list[int],
    ...
    message = f"{indent(depth)}Select cell ({r}, {c}) with {len(candidates)} candidates [exhaustive]"
    trace(trace_enabled, trace_log, message)
    record_step("select_cell", message, row=r, col=c, candidates=candidates)
    ...
    for value in candidates:
        ...
        if search_first_solution_exhaustive(
            grid=grid,
            target=target,
            size=size,
            row_sums=row_sums,
